namespace KDT {
  ...
  void dfs(Node *cur) {   
    if(!cur) return; dfs(cur->ch[0]);
    pool[++tot] = cur; dfs(cur->ch[1]);
  }
  void rebuild(Node *&cur, int d) {
    tot = 0; dfs(cur); cur = build(1, tot, d);
  }
  ...
  void add(int x, int y, int w) { // (x, y) 处加入一个权值为 w 的点
    Node *p = new Node(x, y, w);
    insert(root, p, 0);
  }
  int query(int a, int b, int c, int d) { // 查询 (a, b)-(c, d) 的和
    x0 = a, y0 = b, x1 = c, y1 = d;
    return query(root);
  }
}
